<script lang="ts">
  ...
  let count: Readable<number> | undefined = $state();
  let die: DiceDialog | undefined = $state();
  let currentName: string | undefined = $state();
  let activeChars: string[] | undefined = $state();
  ...
  onMount(() => {
    ...
    const unrollcall = chars.subscribe((v) => {
      ...
      activeChars = Array.from(v.entries())
        .map(([, { name }]) => name)
        .filter((x) => !!x);
  ...
  let buttonText = $derived(count && ($count ?? 0) > 0 ? 'Disconnect' : 'Connect');

  let showImage = $state('');
  function openImg(url: string | undefined) {
    if (!url) return;
...
<div class="px-4 py-6 sm:px-6 flex flex-col gap-4 overflow-y-auto h-[100dvh]">
    ...
    <div class="flex gap-2 items-center">
      {#if activeChars && activeChars.length > 0}
        ...
        {:else if activeChars.length > 1}
          <select
            ...
            class="rounded-md pl-2 py-1 border-orange-500 dark:bg-gray-900"
          >
      ...
      {:else}
        <button
          type="button"
          onclick={() => rerequest()}
          class="rounded-md px-2 py-1 bg-orange-300 dark:bg-orange-700">Check Characters</button
        >
      {/if}
    </div>
  ...
  <div class="flex gap-3 items-center">
    <input
      ...
      name="peerId"
      spellcheck="false"
      readonly={count && ($count ?? 0) > 0}
      placeholder="Game Code (from GM)"
      bind:value={$gameId}
      class="rounded-md flex-1 px-2 py-1 dark:text-white focus:ring-orange-500 focus:border-orange-500 dark:bg-gray-900 {count &&
      ($count ?? 0) > 0
        ? `opacity-50`
        : ``}"
    />
    <button
      type="button"
      onclick={connect}
      class="rounded-md px-2 py-1 bg-orange-300 dark:bg-orange-700">{buttonText}</button
    >
  </div>
  ...
  {:else}
    <div
      class="text-gray-500 border-3 border-dashed rounded-lg p-4 border-gray-500 h-full flex flex-col gap-4 items-center justify-center text-center"
    >
      <Icon icon="nav-arcana" class="size-8" />
