...
<main class="flex flex-col min-h-screen min-h-[100svh]">
  <header class="shrink-0 bg-gray-300 dark:bg-gray-900">
    <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      <h1 class="font-title text-3xl text-center"><a href="https://brighterworldsrpg.com" class="text-purple-700 dark:text-purple-300">Brighter Worlds</a> GM&nbsp;Tools</h1>
      ...
      <div class="px-4 py-6 sm:px-6 lg:pl-8 xl:flex-1 xl:pl-6 flex flex-col gap-4">
        ...
        {#each $mat as item}
          ...
          {:else if item.type === 'npc'}
            <NpcSheet bind:npc={item.npc} on:confirm={() => removeItem(mat, item)} />
          {:else if item.type === 'item'}
            <Disclosable>
              <svelte:fragment slot="header">
                <h3 class="text-xl font-subtitle leading-6 flex-1">{item.item.name}</h3>
