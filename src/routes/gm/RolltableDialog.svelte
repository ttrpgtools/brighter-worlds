<script lang="ts">
  ...
  let { npcList = [], encList = [], relicList = [] }: Props = $props();
  ...
  function editForm(existing: CustomRolltableDef) {
    ...
    const realOptions = options.map((opt) =>
      opt.type === 'text'
        ? opt
        : {
            ...opt,
  ...
  let valid = $derived(!!form.name && !!form.formula && form.options.length > 0);
...
>
  <form class="text-center flex flex-col gap-2">
    ...
    <div class="flex justify-between">
      ...
      <Button class="border-0" icon="plus" size="tight" iconClass="size-4" onclick={addOption} />
    </div>
    {#each form.options as opt, oi}
      <div class="flex gap-2">
        ...
        <div>
          <select
            bind:value={form.options[oi].type}
            class="rounded-full dark:bg-gray-900 dark:text-white focus:ring-purple-500 focus:border-purple-500"
          >
        ...
        <div class="flex-1 flex-shrink">
          {#if form.options[oi].type === 'text'}
            <input
              ...
              name="value"
              placeholder="Value"
              bind:value={form.options[oi].value}
              class="rounded-full dark:bg-gray-900 dark:text-white focus:ring-purple-500 focus:border-purple-500"
            />
          {:else}
            <select
        ...
        <div>
          <input
            ...
            size="6"
            placeholder="Trigger"
            bind:value={form.options[oi].trigger}
            class="rounded-full dark:bg-gray-900 dark:text-white focus:ring-purple-500 focus:border-purple-500"
          />
        </div>
      </div>
