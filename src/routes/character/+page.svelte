<script lang="ts">
  ...
  import ButtonLink from '$lib/ui/button.svelte';
        ...
        <Card class="w-full sm:w-1/3 min-w-[22rem]">
          <div class="flex flex-col gap-4">
            ...
            <div class="flex gap-16 justify-center">
              <div class="relative">
                <span
                  class="absolute font-black tracking-tighter text-3xl text-purple-200 dark:text-purple-800 left-1/2 -translate-x-1/2 top-1"
                  >STR</span
                >
              ...
              <div class="flex flex-row gap-4 items-center">
                ...
                <div>
                  <Button
                    size="icon"
                    icon="download"
                    title="Download {sheet.name || '?'} (JSON)"
                    onclick={() => downloadSheet(sheet.id)}
                  />
                </div>
              </div>
              <DeleteButton onconfirm={() => deleteCharacter(sheet.id)}></DeleteButton>
            </div>
          </div>
        </Card>
      {/each}
  ...
  <div class="flex gap-4 items-center">
    ...
    <div>
      <Button
        size="icon"
        icon="upload"
        title="Upload character from JSON file"
        onclick={startUpload}
      />
      <input
        type="file"
