<script lang="ts">
  ...
  function startWizard(calling: Calling) {
    track(`v1 start wizard: ${calling.name}`);
...
<div class="max-w-prose text-left mb-6 text-md">
  All characters are generalists. Everyone can swing a sword, sneak around, or cast a spell (given
  access to a Grimoire). These Callings define the things that put you ahead of the pack, give you
  an extra edge or let you break the rules.
</div>
<div class="rounded-md bg-yellow-100 dark:bg-amber-800 p-4 mb-6 max-w-prose">
  <div class="flex">
    ...
    <div class="ml-3 text-sm text-yellow-800 dark:text-yellow-100">
      <p>
        Note that for the moment, any characters created here are saved locally in this browser.
        They will not be available on another device or browser.
      </p>
    </div>
  </div>
</div>
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 max-w-7xl">
  {#each data.list as calling}
    <CallingSummary {calling} on:click={() => startWizard(calling)} />
  {/each}
</div>
