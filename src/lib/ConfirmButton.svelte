<script lang="ts">
  ...
  let { confirmText, children: subChild, onconfirm }: Props = $props();
</script>

<ConfirmButtonLogic {onconfirm}>
  {#snippet children({ handleClick, handleIn, handleOut, ready, confirming })}
    <span class="relative inline-block">
      ...
      {#if ready}
        <span
          class="helper absolute text-center z-10 bg-rose-500 text-white border-0 rounded text-sm top-1/2 left-0 px-2 py-1 w-max"
          transition:fade={{ duration: 175 }}
        >
          <span
            class="nib absolute z-10 w-0 h-0 top-1/2 right-0 border-y-[0.5rem] border-l-[0.5rem] border-r-0 border-y-transparent border-r-transparent border-l-rose-500 -translate-y-1/2 translate-x-[90%]"
          ></span>
          {confirmText}
        </span>
      {/if}
