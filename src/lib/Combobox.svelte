<script lang="ts">
  ...
  $effect(() => {
    if (selectedValue) combobox.set({ selected: selectedValue });
  });
  ...
  combobox.subscribe((x) => onselect(x.selected));
</script>
...
<div>
  <div class="relative mt-1">
    <input
      ...
      data-1p-ignore
      use:combobox.input
      onchange={() => onselect($combobox.selected)}
      type="text"
    ...
    {#if $combobox.expanded}
      ...
      >
        {#each filtered as value}
          {@const active = $combobox.active?.name === value.name}
          {@const selected = $combobox.selected?.name === value.name}
          <li
            use:combobox.item={{ value }}
            class="relative cursor-default select-none py-2 px-3 {active
              ? 'bg-purple-700 dark:bg-purple-300 text-white dark:text-gray-900'
              : 'text-gray-900 dark:text-white'}"
          >
            <div class="flex gap-2">
              {#if children}
                {@render children({ value, active, selected })}
              {:else}
