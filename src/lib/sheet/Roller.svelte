<script lang="ts">
  ...
  const setupCloser = onClickOutside(() => show = false);
  function roll(die: DieValue) {
    dispatch('roll', die);
    show = false;
  }
  function toggle() {
...
<div use:portal class="fixed bottom-0 inset-x-0">
  <button type="button" on:click={toggle} class="absolute bottom-1 left-1 inline-flex items-center rounded-full bg-purple-300 dark:bg-purple-700 p-2 font-medium shadow-xl dark:shadow-purple-400/20 hover:bg-purple-200 dark:hover:bg-purple-800 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-900"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="h-8 w-8"><path fill="currentColor" d="M463.9 116.1l-192-111.9C267 1.438 261.5 0 255.1 0S245 1.438 240.1 4.25l-192 111.9C38.12 121.1 32 132.7 32 144.5v223c0 11.75 6.125 22.48 16.12 28.36l192 111.9C245 510.6 250.5 512 256 512s11-1.438 15.88-4.25l192-111.9C473.9 390 480 379.3 480 367.5V144.5C480 132.7 473.9 121.1 463.9 116.1zM256 57.6l94.75 118.4H161.2L256 57.6zM156.6 207.1h198.8L256 369.5L156.6 207.1zM224.9 380l-137-17.13l44.5-133.3L224.9 380zM379.6 229.6l44.5 133.3l-137 17.13L379.6 229.6zM386.9 169.9l-92.25-115.1l138 80.76L386.9 169.9zM125.1 169.9L79 135.2l138.5-80.76L125.1 169.9zM109.1 197.9L64 333.3l.25-169.1L109.1 197.9zM240 414.1v56.38L117.4 398.8L240 414.1zM395.6 398.6L272 470.9v-56.76L395.6 398.6zM402.9 197.9L448 163.1l-.25 168.8L402.9 197.9zM256.2 480L256.2 480L256.2 480z"/></svg></button>
  {#if show}
  <div use:closeOut class="absolute bottom-14 left-1 flex gap-6 sm:gap-6 justify-start p-4 flex-row flex-wrap-reverse w-full">
    {#each dice as die, di}
    <button type="button" transition:fly={{ y: 50, duration: 250, delay: di*50 }} on:click={() => roll(die)} class="flex items-center rounded-full bg-purple-300 dark:bg-purple-700 p-2">
