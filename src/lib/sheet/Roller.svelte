<script lang="ts">
  ...
  let { onroll }: { onroll: (die: DieValue) => void } = $props();
...
<div use:portal class="fixed bottom-0 inset-x-0">
  ...
  {#if show}
    <div
      use:closeOut
      class="absolute bottom-14 left-1 flex gap-6 sm:gap-6 justify-start p-4 flex-row flex-wrap-reverse w-full"
    >
      {#each dice as die, di}
        <button
          type="button"
          transition:fly={{ y: 50, duration: 250, delay: di * 50 }}
          onclick={() => roll(die)}
          class="flex items-center rounded-full bg-purple-300 dark:bg-purple-700 p-2"
        >
