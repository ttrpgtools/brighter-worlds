<script lang="ts">
  import type { Attribute, DieRollSet, DieValue } from "$lib/types";
  ...
  function takeDamage(ev: CustomEvent<{dice: DieRollSet}>) {
    dispatch('damage', {type: name.toLowerCase(), mod: ev.detail.dice.mod});
  }
</script>
<div class="grid grid-cols-5 items-center">
  ...
  <div class="flex justify-end text-right">
    <RollSelector label="" direction={-1} die={value.current || 4} let:events on:roll={takeDamage}>
